...
export function PythonSetup({ onReady }: Props) {
  ...
  async function checkPython() {
    dispatch({ type: "SET_STAGE", stage: "checking", message: "Checking for existing installation…" });
    try {
      const result = await api.checkPython();
      if (result.found && result.version) {
        dispatch({ type: "SET_PYTHON_VERSION", version: result.version });
        dispatch({ type: "SET_STAGE", stage: "ready", message: "Installation found. Starting up…" });
        await startBackend();
      } else if (result.bundled) {
        dispatch({ type: "SET_STAGE", stage: "needs_install", message: "Ready to set up." });
      } else {
        dispatch({ type: "SET_STAGE", stage: "needs_download", message: "One-time setup required." });
      }
    } catch (e) {
  ...
  async function startBackend() {
    dispatch({ type: "SET_STAGE", stage: "backend_starting", message: "Starting up…" });
    try {
      const result = await api.startBackend();
      if (result.ok) {
        dispatch({ type: "SET_STAGE", stage: "backend_ready", message: "Ready!" });
        setTimeout(() => onReadyRef.current(), 600);
      } else {
        dispatch({ type: "SET_ERROR", detail: result.reason ?? "Failed to start. Try restarting the app." });
      }
    } catch (e) {
      dispatch({ type: "SET_ERROR", detail: String(e) });
    }
        ...
        {state.stage === "needs_download" && (
          <div className="rounded-lg bg-amber-950/30 border border-amber-700/30 p-4 flex gap-3">
            ...
            <div className="text-xs text-amber-200/80 leading-relaxed">
              <p className="font-semibold text-amber-300 mb-1">First-time setup needed</p>
              <p>CADViewer needs to download some components to process CAD files. This is a <strong>one-time step</strong> and only affects this app — nothing is changed on your system.</p>
            </div>
          </div>
        )}

        {/* Info box for needs_install (components are bundled) */}
        {state.stage === "needs_install" && (
          <div className="rounded-lg bg-blue-950/30 border border-blue-700/30 p-4 flex gap-3">
            ...
            <div className="text-xs text-blue-200/80 leading-relaxed">
              <p className="font-semibold text-blue-300 mb-1">First-time setup needed</p>
              <p>CADViewer needs to install some components to process CAD files (~500 MB). This is a <strong>one-time step</strong> and only affects this app — nothing is changed on your system.</p>
            </div>
          </div>
        )}
...
const STEPS: { key: Stage[]; label: string }[] = [
  ...
  { key: ["needs_install", "installing", "install_done"], label: "Set Up" },
  { key: ["ready", "backend_starting", "backend_ready"], label: "Launch" },
];
